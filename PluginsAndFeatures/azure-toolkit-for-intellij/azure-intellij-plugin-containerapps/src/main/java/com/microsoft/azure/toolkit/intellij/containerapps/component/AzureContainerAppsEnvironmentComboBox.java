...
public class AzureContainerAppsEnvironmentComboBox extends AzureComboBox<ContainerAppsEnvironment> {
    ...
    public void setValue(@Nullable ContainerAppsEnvironment val) {
        if (Objects.nonNull(val) && val.isDraftForCreating() && !val.exists() && !draftItems.contains(val)) {
    ...
    private ContainerAppsEnvironmentDraft getEnvironmentDraft() {
        ...
        final String defaultEnvironmentName = String.format("ace-%s-%s", resourceGroup.getName(), UUID.randomUUID().toString());
    ...
    private LogAnalyticsWorkspaceDraft getWorkspaceDraft() {
        ...
        final String defaultName = String.format("workspace-%s-%s", resourceGroup.getName(), UUID.randomUUID().toString());
